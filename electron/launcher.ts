...
const runningGames = new Map<number, { process: ChildProcess; startTime: number; sessionId: number }>();

export function killAllGames(db: Database.Database) {
...
export function launchGame(
    ...
    if (runningGames.has(gameId)) return false;

    const startTime = Date.now();

    // Check for emulator
    const game = db.prepare('SELECT emulator_id, rom_path FROM games WHERE id = ?').get(gameId) as { emulator_id: number | null, rom_path: string | null } | undefined;

    let targetExe = exePath;
    let args: string[] = [];

    if (game?.emulator_id && game.rom_path) {
        const emulator = db.prepare('SELECT exe_path, args_template FROM emulators WHERE id = ?').get(game.emulator_id) as { exe_path: string, args_template: string } | undefined;
        if (emulator) {
  ...
  `).run(gameId);
    ...
    gameProcess.on('exit', () => {
        if (!runningGames.has(gameId)) return; // Already cleaned up e.g. on quit
...
export function getRunningGame(gameId: number): { startTime: number } | null {
